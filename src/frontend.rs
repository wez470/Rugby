...
pub fn start_frontend(cpu: &mut Cpu, inst_limit: Option<usize>) {
    ...
    let window = glutin::WindowBuilder::new()
        .with_title("Rustboy")
        .with_dimensions(LogicalSize::new(1024.0, 1024.0))
        .with_resizable(true);
    let context = glutin::ContextBuilder::new().with_vsync(true);
    let (window, mut device, mut factory, rtv, _stv) =
        gfx_window_glutin::init::<Rgba8, DepthStencil>(window, context, &events_loop)
        .expect("Failed to initialize gfx glutin backend");
    let mut encoder = gfx::Encoder::from(factory.create_command_buffer());

    let mut imgui = imgui::ImGui::init();
    imgui.set_ini_filename(None);
    let mut imgui_winit = imgui_winit::ImGuiWinit::new(&mut imgui);
    ...
    let mut imgui_renderer = imgui_gfx_renderer::Renderer::init(
        &mut imgui,
        &mut factory,
        imgui_gfx_renderer::Shaders::GlSl110,
        rtv.clone(),
    ...
    'main: for inst_count in 0.. {
        ...
        events_loop.poll_events(|event| {
            ...
            match event {
                glutin::Event::WindowEvent { event: win_event, .. } => match win_event {
        ...
        // // TODO(solson): As above, replace `gl` with `glium` to avoid unsafe.
        // unsafe {
        ...
        // let frame_size = imgui_glutin_support::get_frame_size(&window, hidpi_factor)
        // let ui = imgui.frame(frame_size, delta_s);

        let ui = imgui_winit.frame(&mut imgui, &window);
        ui.show_demo_window(&mut true);

        encoder.clear(&rtv, [0.0, 0.0, 0.0, 0.0]);
        imgui_renderer.render(ui, &mut factory, &mut encoder).unwrap();
        ...
        assert_eq!((Instant::now() - frame_start_time).as_secs(), 0);
        log::trace!("Frame time: {}ms", (Instant::now() - frame_start_time).subsec_millis());
